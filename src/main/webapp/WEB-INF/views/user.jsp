  ...
  <body>
	...
	<div class="button">
	  <a href="<c:url value="/payment/service"/>">
	    <img src="<%= request.getContextPath() %>/images/communal.png"><br><spring:message code="label.PaymentforService"/>
	  </a>
	</div>
      </c:if>
      <c:if test="${admin==null || !admin}">
	...
	<div id="pay_for_services" class="section">
	  ...
	  <form:form method="post" action="${pageContext.request.contextPath}/account/moveBalance/Service" commandName="money">
		...
		<td><input class="summ" type="text" size="10" name="money"></td>
	      ...
	      <tr>
		<td colspan="2" align="center"><input type="submit" value="<spring:message code="label.Submit"/>"></td>
	      </tr>
	    </table>
	  </form:form>
	</div>
		...
		<tr>
		  ...
		  <td>
		    ...
		    <c:if test="${transactionType.equals('INCREASE_ACCOUNT_FROM')}">
		      <spring:message code="label.PaymentFrom"/>
		    </c:if>
		  </td>
		</tr>
	      </c:forEach>
